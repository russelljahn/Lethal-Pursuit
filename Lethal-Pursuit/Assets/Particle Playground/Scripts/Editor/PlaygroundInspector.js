...
class PlaygroundInspector extends Editor {
	...
	static function RenderPlaygroundSettings () {
		...
		if (playgroundFoldout && playgroundScriptReference!=null) {
			...
			if (advancedSettingsFoldout) {
				...
				// Limits
				EditorGUILayout.BeginVertical(boxStyle);
				if (GUILayout.Button((limitsFoldout?"▼":"►")+" Editor Limits", EditorStyles.label)) limitsFoldout=!limitsFoldout;
				if (limitsFoldout) {
					EditorGUILayout.Separator();
	...
	static function RenderManipulatorSettings (thisManipulator : ManipulatorObject, serializedManipulator : SerializedProperty, isPlayground : boolean) {
		...
		if (thisManipulator.type==MANIPULATORTYPE.Property) {
			...
			// Target
			if (thisManipulator.property.type==MANIPULATORPROPERTYTYPE.Target) {
				
				// Target List
				var hasNull : boolean = false;
				EditorGUILayout.BeginVertical(boxStyle);
				if (GUILayout.Button((targetsFoldout?"▼":"►")+" Targets ("+thisManipulator.property.targets.Count+")", EditorStyles.label)) targetsFoldout=!targetsFoldout;
				if (targetsFoldout) {
					if (thisManipulator.property.targets.Count>0) {
						for (var t = 0; t<thisManipulator.property.targets.Count; t++) {
							EditorGUILayout.BeginVertical(boxStyle, GUILayout.MinHeight(26));
							EditorGUILayout.BeginHorizontal();
							
							GUILayout.Label(t.ToString(), EditorStyles.miniLabel, GUILayout.Width(18));
							thisManipulator.property.targets[t] = EditorGUILayout.ObjectField("", thisManipulator.property.targets[t], Transform, true) as Transform;
							if (!thisManipulator.property.targets[t]) hasNull = true;
								
							EditorGUILayout.Separator();
							if(GUILayout.Button("-", EditorStyles.toolbarButton, [GUILayout.Width(18), GUILayout.Height(16)])){
								thisManipulator.property.targets.RemoveAt(t);
							}
					...
					if(GUILayout.Button("Create", EditorStyles.toolbarButton, GUILayout.Width(50))){
						thisManipulator.property.targets.Add(thisManipulator.transform);
					}
					EditorGUILayout.Separator();
				}
				EditorGUILayout.EndVertical();
				...
				thisManipulator.property.onlyPositionInRange = EditorGUILayout.Toggle("Only Position In Range", thisManipulator.property.onlyPositionInRange);
				thisManipulator.property.zeroVelocityStrength = EditorGUILayout.Slider("Zero Velocity Strength", thisManipulator.property.zeroVelocityStrength, 0, playgroundScriptReference.maximumAllowedManipulatorZeroVelocity);
		...
		serializedManipulatorStrength.floatValue = EditorGUILayout.Slider("Strength", serializedManipulatorStrength.floatValue, 0, playgroundScriptReference.maximumAllowedManipulatorStrength);
