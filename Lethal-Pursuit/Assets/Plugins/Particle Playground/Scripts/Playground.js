...
class Playground extends MonoBehaviour {
	...
	static var version : String = "1.16";
...
class ManipulatorObject {
	...
	function Contains (position : Vector3) : boolean {
		if (shape==MANIPULATORSHAPE.Box) {
			...
			if (!inverseBounds)
				return (Vector3.Distance(position, transform.position)<=size);
			else
				return (Vector3.Distance(position, transform.position)>=size);
		}
		return false;
...
class ManipulatorProperty {
	...
	var targetId : int;									// The target id for this manipulator (to pair particles with this manipulator property)
	...
	function Clone () : ManipulatorProperty {
		...
		manipulatorProperty.targetId = targetId;
		manipulatorProperty.targets = new List.<Transform>();
		for (var i = 0; i<targets.Count; i++)
			manipulatorProperty.targets.Add(manipulatorProperty.targets[i]);
