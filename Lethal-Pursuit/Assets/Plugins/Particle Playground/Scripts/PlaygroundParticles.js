...
class PlaygroundParticles extends MonoBehaviour {
	...
	static function NewManipulatorObject (type : MANIPULATORTYPE, affects : LayerMask, manipulatorTransform : Transform, size : float, strength : float, playgroundParticles : PlaygroundParticles) : ManipulatorObject {
		...
		manipulatorObject.property.targetId = manipulatorTransform.GetInstanceID();
	...
	static function SetLifetime (playgroundParticles : PlaygroundParticles, time : float) {
		if (!playgroundParticles.playgroundCache.targetPosition) return;
	...
	static function SetParticleTimeNow (playgroundParticles : PlaygroundParticles) {
		if (!playgroundParticles.playgroundCache.lifetimeOffset || playgroundParticles.playgroundCache.lifetimeOffset.Length!=playgroundParticles.particleCount) return;
					...
					switch (playgroundParticles.source) {
						...
						case SOURCE.State:
							...
							if (playgroundParticles.states.Count>0 && playgroundParticles.states[playgroundParticles.activeState]!=null && playgroundParticles.states[playgroundParticles.activeState].initialized) {
								...
								if (playgroundParticles.applyInitialLocalVelocity && playgroundParticles.playgroundCache.life[p]==0 && playgroundParticles.states[playgroundParticles.activeState].stateTransform!=null && !playgroundParticles.onlySourcePositioning) {
									playgroundParticles.playgroundCache.initialLocalVelocity[p] = playgroundParticles.states[playgroundParticles.activeState].stateTransform.TransformDirection(
						...
						case SOURCE.Transform:
							...
							if (playgroundParticles.sourceTransform!=null) {
								...
								if (playgroundParticles.overflowOffset!=Vector3.zero)
									...
									playgroundParticles.playgroundCache.targetPosition[p] = playgroundParticles.sourceTransform.position;
								...
								if (playgroundParticles.applyInitialLocalVelocity && playgroundParticles.playgroundCache.life[p]==0 && !playgroundParticles.onlySourcePositioning) {
									playgroundParticles.playgroundCache.initialLocalVelocity[p] = playgroundParticles.sourceTransform.TransformDirection(
										Vector3(UnityEngine.Random.Range(playgroundParticles.initialLocalVelocityMin.x, playgroundParticles.initialLocalVelocityMax.x),
												UnityEngine.Random.Range(playgroundParticles.initialLocalVelocityMin.y, playgroundParticles.initialLocalVelocityMax.y),
												UnityEngine.Random.Range(playgroundParticles.initialLocalVelocityMin.z, playgroundParticles.initialLocalVelocityMax.z)
										)
						...
						case SOURCE.WorldObject:
							...
							if (playgroundParticles.worldObject.gameObject!=null) {
								...
								if (playgroundParticles.applyInitialLocalVelocity && playgroundParticles.playgroundCache.initialLocalVelocity[p]!=Vector3.zero && playgroundParticles.playgroundCache.life[p]==0 && !playgroundParticles.onlySourcePositioning) {
									playgroundParticles.playgroundCache.initialLocalVelocity[p] = playgroundParticles.worldObject.transform.TransformDirection(
						...
						case SOURCE.SkinnedWorldObject:
							...
							if (playgroundParticles.skinnedWorldObject.gameObject!=null) {
								...
								// Normal direction velocity
								if (playgroundParticles.applyInitialLocalVelocity && playgroundParticles.playgroundCache.initialLocalVelocity[p]!=Vector3.zero && playgroundParticles.playgroundCache.life[p]==0 && !playgroundParticles.onlySourcePositioning) {
									playgroundParticles.playgroundCache.initialLocalVelocity[p] = playgroundParticles.skinnedWorldObject.transform.TransformDirection(
										Vector3(playgroundParticles.skinnedWorldObject.normals[p%playgroundParticles.skinnedWorldObject.normals.Length].x*UnityEngine.Random.Range(playgroundParticles.initialLocalVelocityMin.x, playgroundParticles.initialLocalVelocityMax.x),
												playgroundParticles.skinnedWorldObject.normals[p%playgroundParticles.skinnedWorldObject.normals.Length].y*UnityEngine.Random.Range(playgroundParticles.initialLocalVelocityMin.y, playgroundParticles.initialLocalVelocityMax.y),
												playgroundParticles.skinnedWorldObject.normals[p%playgroundParticles.skinnedWorldObject.normals.Length].z*UnityEngine.Random.Range(playgroundParticles.initialLocalVelocityMin.z, playgroundParticles.initialLocalVelocityMax.z)
										)
									);
									playgroundParticles.playgroundCache.velocity[p] += playgroundParticles.playgroundCache.initialLocalVelocity[p];
								}
						...
						case SOURCE.Paint:
							...
							if (playgroundParticles.paint!=null && playgroundParticles.paint.positionLength>0) {
								...
								if (playgroundParticles.applyInitialLocalVelocity && playgroundParticles.playgroundCache.initialLocalVelocity[p]!=Vector3.zero && playgroundParticles.playgroundCache.life[p]==0 && !playgroundParticles.onlySourcePositioning) {
									...
									playgroundParticles.playgroundCache.initialLocalVelocity[p] = playgroundParticles.paint.GetParent(p).TransformDirection(
		...
		if (thisManipulator.enabled && thisManipulator.transform!=null && thisManipulator.strength!=0 && (thisManipulator.affects.value & 1<<playgroundParticles.particleSystemGameObject.layer)!=0) {
			...
			if (!playgroundParticles.onlySourcePositioning) {
				...
				if (thisManipulator.type==MANIPULATORTYPE.Attractor) {
					...
					if (thisManipulator.Contains(playgroundParticles.particleCache.particles[p].position)) {
				...
				if (thisManipulator.Contains(playgroundParticles.particleCache.particles[p].position)) {
					...
					switch (thisManipulator.property.type) {
						...
						case MANIPULATORPROPERTYTYPE.Size:
							...
							else if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Lerp)
							...
							else if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Linear)
								playgroundParticles.particleCache.particles[p].size = Mathf.MoveTowards(playgroundParticles.particleCache.particles[p].size, thisManipulator.property.size, t*thisManipulator.strength);
							playgroundParticles.playgroundCache.changedByPropertySize[p] = true;
						break;
						
						// Target Property
						case MANIPULATORPROPERTYTYPE.Target:
							if (thisManipulator.property.targets.Count>0 && thisManipulator.property.targets[thisManipulator.property.targetPointer%thisManipulator.property.targets.Count]) {
								
								// Check id pairer
								if (thisManipulator.transform.GetInstanceID()!=thisManipulator.property.targetId) {
									thisManipulator.property.targetId = thisManipulator.transform.GetInstanceID();
								}
								
								// Set target pointer
								if (playgroundParticles.playgroundCache.propertyId[p] != thisManipulator.property.targetId) {
									playgroundParticles.playgroundCache.propertyTarget[p] = thisManipulator.property.targetPointer;
									thisManipulator.property.targetPointer++; thisManipulator.property.targetPointer=thisManipulator.property.targetPointer%thisManipulator.property.targets.Count;
									playgroundParticles.playgroundCache.propertyId[p] = thisManipulator.property.targetId;
								}
								
								// Teleport or lerp to position based on transition type
								if (playgroundParticles.playgroundCache.propertyId[p] == thisManipulator.property.targetId && thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count]) {
									if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.None)
										playgroundParticles.particleCache.particles[p].position = thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count].position;
									else if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Lerp) {
										playgroundParticles.particleCache.particles[p].position = Vector3.Lerp(playgroundParticles.particleCache.particles[p].position, thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count].position, t*thisManipulator.strength);
										if (thisManipulator.property.zeroVelocityStrength>0)
											playgroundParticles.playgroundCache.velocity[p] = Vector3.Lerp(playgroundParticles.playgroundCache.velocity[p], Vector3.zero, t*thisManipulator.property.zeroVelocityStrength);
									} else if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Linear) {
										playgroundParticles.particleCache.particles[p].position = Vector3.MoveTowards(playgroundParticles.particleCache.particles[p].position, thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count].position, t*thisManipulator.strength);
										if (thisManipulator.property.zeroVelocityStrength>0)
											playgroundParticles.playgroundCache.velocity[p] = Vector3.Lerp(playgroundParticles.playgroundCache.velocity[p], Vector3.zero, t*thisManipulator.property.zeroVelocityStrength);
									}
					...
					// Particle is outside of property range and is set to only color in range (try to lerp back color)
					...
					if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Lerp || thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Linear) {
						if (playgroundParticles.playgroundCache.changedByPropertyTarget[p] && !thisManipulator.property.onlyPositionInRange && thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count] && playgroundParticles.playgroundCache.propertyId[p] == thisManipulator.property.targetId) {
							if (thisManipulator.property.transition == MANIPULATORPROPERTYTRANSITION.Lerp)
								playgroundParticles.particleCache.particles[p].position = Vector3.Lerp(playgroundParticles.particleCache.particles[p].position, thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count].position, t*thisManipulator.strength);
							else
								playgroundParticles.particleCache.particles[p].position = Vector3.MoveTowards(playgroundParticles.particleCache.particles[p].position, thisManipulator.property.targets[playgroundParticles.playgroundCache.propertyTarget[p]%thisManipulator.property.targets.Count].position, t*thisManipulator.strength);
	...
	static function Rebirth (playgroundParticles : PlaygroundParticles, p : int) {
		...
		if (playgroundParticles.applyInitialVelocity && !playgroundParticles.onlySourcePositioning) {
			playgroundParticles.playgroundCache.initialVelocity[p] = Vector3(
...
class PlaygroundCache extends System.ValueType {
	...
	var color : Color32[];						// The color of each particle in this PlaygroundParticles
	...
	var changedByPropertyTarget : boolean[];	// The interaction with property manipulators that change target of each particle
	var propertyTarget : int[];					// The property target pointer for each particle
	var propertyId : int[];						// The property target id for each particle (pairing this particles to a manipulator)
